...
function App() {
  ...
  <div className=' flex gap-5   '>
      ...
      <div className=' bg-neutral-800 p-6 shadow-md '>
        ...
        <ol className=''>
          {task.map((task ,index)=> {
            return <li className='px-[5px] py-[2.5px] flex items-center gap-x-1 '  key={index}>
              <span className='flex-1'>{task}</span>
              <button className='w-20' onClick={()=>handleDelete(index)} >Delete</button>
              <ArrowUpwardIcon className='  hover:cursor-pointer' onClick={()=>handleUp(index)} >UP</ArrowUpwardIcon>
              <ArrowDownwardIcon className='  hover:cursor-pointer' onClick={()=>handleDown(index)} >Down</ArrowDownwardIcon>
